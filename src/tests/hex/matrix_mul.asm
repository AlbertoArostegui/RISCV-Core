i_loop:
    li x1, 0                ; i = 0 (outer loop counter)
j_loop:
    li x2, 0                ; j = 0 (middle loop counter)
    li x18, 0               ; c[i][j] = 0 (accumulator for the dot product)
k_loop:
    li x7, 0                ; k = 0 (inner loop counter)
    li x4, 128              ; Set the matrix dimension (128)

    bge x7, x4, end_k       ; if k >= 128, exit the inner loop

    li x8, 992              ; Load base address of matrix a
    mul x9, x1, x4          ; Calculate row offset: i * 128
    add x10, x9, x7         ; Add column offset: i * 128 + k
    slli x10, x10, 2        ; Convert to byte offset: (i * 128 + k) * 4
    add x8, x8, x10         ; Address of a[i][k]
    lw x11, 0(x8)           ; Load a[i][k]
